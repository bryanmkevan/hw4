### KRISTEN COMPLETED BLOG POST "A Bayesian approach to obtaining uncertainty 
## estimates from neural networks" ON MARCH 14 2019
## Keydana (2018, Nov. 12). TensorFlow for R: You sure? A Bayesian approach 
## to obtaining uncertainty estimates from neural networks. Retrieved from 
...
# NEEDED TO LOAD THE 1.4 VERSION FOR SOME LATER THINGS TO WORK
# install.packages("tensorflow", version=1.4)
install_tensorflow()
library(tensorflow)
...
# LOG OF VARIANCE FOR NUMERIC STABILITY
...
## Yup that was the issue. Thanks a lot. Updated keras to 2.1.3 and Tensorflow 
# to 1.4 ; everything is working fine. Closing the issue.
sessionInfo()
install_keras()
library(keras)
install_tensorflow()
...
# The X axis is the validation data and the confidence interval is the prediction
# confidence intervale for the mean. 
# Places with more data have lower std whereas the edges have a higher std 
...
### makes nice plot from sheeet 1 to show what the variables look like
df <- read_xlsx("/home/kristenmae/biostat-m280-2019-winter/hw4/kristen_work/Sheet1.xlsx")
ggscatmat(df)
...
### seeing the prediction on a real dataset
ggplot(preds, aes(x1, y_pred)) + 
  geom_point() + 
  geom_ribbon(aes(ymin = e_u_lower, ymax = e_u_upper), alpha = 0.3)
...
### Assess Occurrence of Viral Pathogens in Multiple US Drinking Water Systems ###
air <- read_xlsx("/home/kristenmae/biostat-m280-2019-winter/hw4/kristen_work/AirQualityUCI.xlsx")
ggscatmat(air)

# Contains the responses of a gas multisensor device deployed on the field in an Italian city.
# the objective is to predict the CO concentration from other air detection values 
...
df_scaled <- scale(air)
## to predict CO using other variables 
...
### not the best model but trained some. 
...
### Below is plot showing the predicted and actual value of 
# True hourly averaged concentration CO in mg/m^3 
ggplot(preds, aes(x1, y_pred)) + 
  geom_point() + 
  geom_ribbon(aes(ymin = e_u_lower, ymax = e_u_upper), alpha = 0.3) +
  xlab("True hourly averaged concentration CO in mg/m^3") +
  ylab("Predicted hourly average concentration CO") +
  labs(caption= "data from https://archive.ics.uci.edu/ml/datasets/Air+Quality#", 
       title="Predicting CO concentration allowing Heteroskedastic \n variance in Bayesian Model")

## model did really bad job with one area of predictions, which are values between 0.25 and 0.75
