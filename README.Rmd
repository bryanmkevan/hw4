...
folder of the GitHub repository, along with .Rmd and html files titled stephenHw4.Rmd and stephenHw4.html. (Note: eval=FALSE has been set in the code blocks, since re-running the code would require a lot of time.)

2. I added a credits button to the Google Cloud Vision Android app (discussed under Suneeta Godbole's contributions),  that, when clicked, displays a message containing the purpose of the project and the names of the team members who worked on it.
...
I retrained a pretrained neural net on a dataset of images of 100 types of Japanese food obtained from:

http://foodcam.mobi/dataset100.html

and installed it in a prebuilt Android image classification app.

The app lists the three most likely predicted classes of food present in the camera's field of vision, in real time. The app can take a picture of the current image, including the predicted classes for that image.
...
folder on the GitHub repository. Inside, locate a folder titled "app". Build this "app" folder in Android Studio to run it.
