---
title: "Biostatistics M280 final project, Q1: Representation learning with MMD-VAE"
author: Stephen Kwok <sdkwok@ucla.edu>
...
**SOLUTION: We reproduce the following RStudio blog post:**

https://blogs.rstudio.com/tensorflow/posts/2018-10-22-mmd-vae/

**This post uses variational autoencoders (VAE) to model a space of images (from Fashion MNIST, a dataset consisting of images of clothing). Two different approaches to VAE are taken in this post: a standard VAE using ELBO (evidence-based lower bound) maximization, and an Info-VAE using MMD (maximum mean discrepancy), a method which measures the similarity of two probability distributions based on matching their moments.**

## Step 1: Replication of blog results.

Below is the code for the standard VAE, taken from the link at the above blog.
...
library(tensorflow)
#tfe_enable_eager_execution(device_policy = "silent") #turned off!
...
We now add the sample code for the VAE-MMD.
...
use_implementation("tensorflow")
library(tensorflow)
#tfe_enable_eager_execution(device_policy = "silent") #turn off eager
...
**SOLUTION: We will try both VAEs on a different dataset, the CIFAR-10 dataset, from ``Learning Multiple Layers of Features from Tiny Images, Alex Krizhevsky, 2009.'' The images need to be converted to grayscale. **
...
fashion <- dataset_cifar10()

c(train_images, train_labels) %<-% fashion$train
c(test_images, test_labels) %<-% fashion$test

train_x <-
  train_images %>% `/`(255) %>% k_reshape(c(50000, 32, 32, 3))
test_x <-
  test_images %>% `/`(255) %>% k_reshape(c(10000, 32, 32, 3))
...
generate_random_clothes <- function(epoch) {
  predictions <-
    decoder(random_vector_for_generation) %>% tf$nn$sigmoid()
  png(paste0("cvae_cifar_epoch_", epoch, ".png"))
  par(mfcol = c(8, 8))
  par(mar = c(0.5, 0.5, 0.5, 0.5),
      xaxs = 'i',
      yaxs = 'i')
...
show_latent_space <- function(epoch) {
  ...
  ggsave(
    paste0("cvae_cifar_latentspace_epoch_", epoch, ".png"),
    width = 10,
    height = 10,
    units = "cm"
  )
...
show_grid <- function(epoch) {
  png(paste0("cvae_cifar_grid_epoch_", epoch, ".png"))
  par(mar = c(0.5, 0.5, 0.5, 0.5),
      xaxs = 'i',
      yaxs = 'i')
  n <- 16
